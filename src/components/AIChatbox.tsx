...
const knowledgeBase: Record<
  string,
  { keywords: string[]; weight?: number; responses: string[] }
> = {
  // ── ABOUT / INTRODUCTION ──────────────────────────────────
  about: {
    keywords: [
...
const suggestionPool: Suggestion[] = [
  ...
  { label: "🧪 Design Experiments", value: "What are her design experiments?" },
  {
    ...
    value: "What were the budgeting app results?",
  },
  { label: "🔍 User Research", value: "How did she conduct user research?" },
  {
    label: "⚡ Problem Solved",
    value: "What problem does the budgeting app solve?",
  },
  {
    label: "🎓 Engineering Background",
    value: "Tell me about her engineering background",
  },
...
function getResponse(query: string): string {
  ...
  const responses =
    categoryData && categoryData.responses
      ? categoryData.responses
      : [
          "I'm not quite sure how to help with that. Can you try asking about her projects or experience?",
        ];
...
function getDynamicSuggestions(currentValue: string): Suggestion[] {
  ...
  let filtered = suggestionPool.filter(
    (s) =>
  ...
  if (filtered.length < 3) {
    filtered = suggestionPool.filter(
...
const AIChatbox = forwardRef<AIChatboxHandle, AIChatboxProps>(
  function AIChatbox({ onClose, onVoiceStateChange }, ref) {
    ...
    const setAndNotifyVoiceState = useCallback(
    ...
    const startVoice = useCallback(() => {
      if (!projectNarrative) {
        addMessage(
          "Quick Brief is available on individual project pages only.",
        );
        return;
      }
    ...
    const sendMessage = useCallback(
      (customText?: string) => {
        ...
        setTimeout(
          ...
          400 + Math.random() * 400,
        ); // randomize slightly to feel more natural
      },
    ...
    const initSpeechRecognition = useCallback(() => {
      ...
      recognition.onerror = (event: SpeechRecognitionErrorEventLike) => {
        ...
        if (event.error === "not-allowed") {
          setMicStatus({
            text: "⚠️ Mic access denied. Check browser permissions.",
            color: "#ef4444",
          });
    ...
    const toggleMicInput = useCallback(() => {
      ...
      if (!recognitionRef.current) {
        ...
        if (!supported) {
          setMicStatus({
            text: "⚠️ Voice input not supported in this browser.",
            color: "#ef4444",
          });
          setTimeout(() => setMicStatus({ text: "", color: "" }), 3000);
          return;
        ...
        <div className="p-6 border-b border-[var(--theme-border)] bg-[var(--theme-surface)] flex items-center justify-between backdrop-blur-xl">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-[var(--theme-text-hi)] font-bold text-[15px]">
                Shvetha&apos;s AI
              </span>
            ...
            >
              <div
                className={`p-3.5 rounded-[18px] text-[13px] leading-[1.6] shadow-md ${
                  msg.isUser
                    ? "bg-[var(--theme-surface)] border border-[var(--theme-accent)] text-[var(--theme-text-hi)]"
                    : "bg-[var(--theme-surface)] border border-[var(--theme-border)] text-[var(--theme-text-mid)]"
                }`}
